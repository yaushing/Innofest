...
struct LocationView: View {
    ...
    var body: some View {
        VStack {
            ...
            // Align to left side
            HStack {
                Text("Photo Credit: \(photoCredit)").font(.system(size: 10)).italic().foregroundColor(Color.gray)
                Spacer()
            }
            ...
            // Exact Address (SG XXXXXX)
            HStack {
                Text("Address: ")
                Link(exactLoc, destination: URL(string: gmapLink)!)
            }
            
            // Link to activeSG booking
            Link("Website to book programmes", destination: URL(string: website)!)
            
            // Link to a view that shows the sports here
            NavigationLink {
